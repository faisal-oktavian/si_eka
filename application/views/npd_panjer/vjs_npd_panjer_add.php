<script>
	...
	jQuery('#npd_panjer_date').datetimepicker({
	...
	jQuery('body').on('click', '#btn_add_paket_belanja', function() {
		...
		if (npd_panjer_date == '' || npd_panjer_date == null) {
			bootbox.alert('Tanggal NPD Panjer harus diisi.');
		}
		else {
			show_modal('add');

			jQuery('#form_add').find('.detail-paket-belanja').addClass('hide');
			jQuery('#idnpd_panjer_detail').val('');
	...
	jQuery("#search_paket_belanja").select2({
		...
		ajax: { 
		    url: app_url + 'npd_panjer/search_paket_belanja',
		    dataType: "json",
		    delay: 250,
	...
	function seach_paket_belanja(idpaket_belanja) {
		jQuery.ajax({
			url: app_url + 'npd_panjer/select_paket_belanja',
			type: 'POST', 
			dataType: 'JSON',
			data: {
				id: idpaket_belanja
	...
	jQuery('body').on('change', '#iduraian', function() {
		...
		if (idpaket_belanja != "") {
			jQuery.ajax({
				url: app_url + 'npd_panjer/get_paket_belanja_detail_sub_parent',
				type: 'POST',
				dataType: 'JSON',
				data: {
	...
	function get_validate_gender(iduraian) {
		jQuery.ajax({
			url : app_url + 'npd_panjer/get_validate_gender?id='+iduraian,
			method : 'get',
			dataType : 'json',
	...
	function get_validate_description(iduraian) {
		jQuery.ajax({
			url : app_url + 'npd_panjer/get_validate_description?id='+iduraian,
			method : 'get',
			dataType : 'json',
			success : function(res){
				console.log(res);
	...
	function get_validate_room(iduraian) {
		jQuery.ajax({
			url : app_url + 'npd_panjer/get_validate_room?id='+iduraian,
			method : 'get',
			dataType : 'json',
			success : function(res){
				console.log(res);
	...
	function get_validate_name_training(iduraian) {
		jQuery.ajax({
			url : app_url + 'npd_panjer/get_validate_training?id='+iduraian,
			method : 'get',
			dataType : 'json',
			success : function(res){
				console.log(res);
	...
	jQuery('body').on('click', '.btn-action-save', function() {
		...
		if (npd_panjer_date == '' || npd_panjer_date == null) {
			...
			// show_loading();
			jQuery.ajax({
				url: app_url + 'npd_panjer/add_product',
				type: 'POST',
				dataType: 'JSON',
				data: jQuery('#form_add').serialize() + '&npd_panjer_date=' + npd_panjer_date,
				success: function(response) {
					hide_loading();
					if (response.err_code > 0) {
						...
						jQuery('#idnpd_panjer').val(response.idnpd_panjer);
						jQuery('#hd_idnpd_panjer').val(response.idnpd_panjer);

						generate_transaction(response.idnpd_panjer);
					}
				},
				error: function(response) {}
			});
	...
	function generate_transaction(idnpd_panjer) {
		jQuery.ajax({
			url: app_url+'npd_panjer/get_list_order/',
			type: 'POST',
			dataType: 'JSON',
			data: {
	...
	jQuery('body').on('click', '#btn_save_realisasi', function() {
		...
		jQuery.ajax({
			url: app_url + 'npd_panjer/save_realisasi',
			type: 'POST',
			dataType: 'JSON',
			...
			success: function(response) {
				hide_loading();
				if (response.err_code == 0) {
					location.href = app_url + 'npd_panjer';
				}
				else {
					bootbox.alert(response.err_message);
				}
	...
	jQuery('body').on('click','.btn-edit-order', function() {
		...
		show_loading();
		jQuery.ajax({
			url: app_url + 'npd_panjer/edit_order',
			type: 'POST',
			dataType: 'JSON',
			data: {
				id: id
			...
			success: function(response) {
				...
				jQuery('#form_add').find('.detail-paket-belanja').addClass('hide');
				jQuery('#idnpd_panjer_detail').val(id);
				jQuery('#form_add input, #form_add select, #form_add textarea').not('.x-hidden').val('').trigger('change.select2');
	...
	jQuery('body').on('click','.btn-delete-order', function() {
		...
		bootbox.confirm('Apakah anda yakin ingin menghapus data ini?', function(e) {
			...
			if (e) {
				jQuery.ajax({
					url: app_url + 'npd_panjer/delete_order',
					type: 'POST',
					dataType: 'JSON',
					data: {
						id: id
					...
					success: function(response) {
						...
						if(response.err_code == 0) {
							...
							generate_transaction(jQuery('#idnpd_panjer').val(), true);
						} else {
							bootbox.alert(response.err_message);
						}
					},
	...
	if (the_id != "") {
		...
		jQuery.ajax({
			url: app_url + 'npd_panjer/get_data',
			type: 'POST',
			dataType: 'JSON',
			data: {
				id: the_id
