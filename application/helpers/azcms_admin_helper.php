...
defined('BASEPATH') OR exit('No direct script access allowed');   
    ...
    if(!function_exists('az_select_sumber_dana')){
        function az_select_sumber_dana($id = 'sumber_dana', $class='', $attr='sumber_dana') {
            $ci =& get_instance();
            $ci->load->library('encrypt');
            $azapp = $ci->load->library('AZApp');
            $select = $ci->azapp->add_select2();
            $select->set_id($id);
            $select->set_url('data/get_sumber_dana');
            $select->set_placeholder('Pilih Sumber Dana');
            if (strlen($class) > 0) {
                $select->add_class($class);
            }
            if (strlen($attr) > 0) {
                $select->add_attr('data-id', $ci->encrypt->encode($attr.'.idsumber_dana'));
                $select->add_attr('w', 'true');
            }
    ...
    if(!function_exists('az_select_uraian')){
        function az_select_uraian($id = 'paket_belanja_detail_sub', $class='', $attr='paket_belanja_detail_sub') {
            $ci =& get_instance();
            $ci->load->library('encrypt');
            $azapp = $ci->load->library('AZApp');
            $select = $ci->azapp->add_select2();
            $select->set_id($id);
            $select->set_url('data/get_paket_belanja_detail_sub');
            $select->set_placeholder('Pilih Uraian');
            if (strlen($class) > 0) {
                $select->add_class($class);
            }
    ...
    if(!function_exists('az_select_pad_rekening')){
        function az_select_pad_rekening($id = 'pad_rekening', $class='', $attr='pad_rekening') {
            $ci =& get_instance();
            $ci->load->library('encrypt');
            $azapp = $ci->load->library('AZApp');
            $select = $ci->azapp->add_select2();
            $select->set_id($id);
            $select->set_url('data/get_pad_rekening');
            $select->set_placeholder('Pilih Rekening');
            if (strlen($class) > 0) {
                $select->add_class($class);
            }
            if (strlen($attr) > 0) {
                $select->add_attr('data-id', $ci->encrypt->encode($attr.'.idpad_rekening'));
                $select->add_attr('w', 'true');
            }
