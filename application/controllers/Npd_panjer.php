...
class Npd_panjer extends CI_Controller {
	...
	public function index() {		
		...
		$crud->set_column(array('#', 'Tanggal NPD', 'Nomor NPD', 'Paket Belanja', 'Total', 'Status', 'Admin', azlang('Action')));
	...
	public function get() {
        ...
        $crud->set_select('npd_panjer.idnpd_panjer, date_format(npd_panjer_date, "%d-%m-%Y %H:%i:%s") as txt_npd_panjer_date, npd_panjer_code, paket_belanja.nama_paket_belanja, total_realisasi, npd_panjer_status, user.name as user_created');
        $crud->set_select_table('idnpd_panjer, txt_npd_panjer_date, npd_panjer_code, nama_paket_belanja, total_realisasi, npd_panjer_status, user_created');
        $crud->set_sorting('npd_panjer_date, npd_panjer_code, nama_paket_belanja, total_realisasi, npd_panjer_status');
        $crud->set_filter('npd_panjer_date, npd_panjer_code, nama_paket_belanja, total_realisasi, npd_panjer_status');
		$crud->set_id($this->controller);
		...
		$crud->set_group_by('npd_panjer.idnpd_panjer, npd_panjer.npd_panjer_date, npd_panjer_code, paket_belanja.nama_paket_belanja, total_realisasi, user.name');
        ...
        if (strlen($npd_panjer_code) > 0) {
			$crud->add_where('npd_panjer.npd_panjer_code = "' . $npd_panjer_code . '"');
		}
	...
	{	
		...
		if ($err_code == 0) {
			...
			$this->db->select('npd_panjer_detail.idnpd_panjer_detail, kode_rekening.kode_rekening, sub_kategori.nama_sub_kategori as nama_uraian, npd_panjer_detail.total, npd_panjer_detail.npd_detail_description, npd_panjer.total_realisasi, date_format(npd_panjer.npd_panjer_date, "%d %M %Y") as npd_panjer_date');
			...
			$npd_panjer_date = '';
			$arr_data = array();
			foreach ($panjer_detail->result() as $key => $value) {
				$arr_data[] = array(
					'kode_rekening' => $value->kode_rekening,
					'nama_uraian' => $value->nama_uraian,
					'total' => $value->total,
					'description_detail' => $value->npd_detail_description,
				);

				$total_realisasi = floatval($value->total_realisasi);
				$npd_panjer_date = $value->npd_panjer_date;
			}
	...
	function get_data() {
		...
		$this->db->select('date_format(npd_panjer_date, "%d-%m-%Y %H:%i:%s") as txt_npd_panjer_date, npd_panjer_code, user.name as user_created, npd_panjer.iduser_created');
