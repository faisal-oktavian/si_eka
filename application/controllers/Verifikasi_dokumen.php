...
class Verifikasi_dokumen extends CI_Controller {
	...
	public function index() {		
		...
		$crud->set_column(array('#', 'Tanggal Input Dokumen', 'Tanggal Verifikasi Dokumen', 'Nomor Dokumen', 'Detail', 'Keterangan Verifikasi', 'User Input', 'User Verifikasi', azlang('Action')));
	...
	public function get() {
        ...
        $crud->set_select('verification.idverification, date_format(verification_date_created, "%d-%m-%Y %H:%i:%s") as txt_date_input, date_format(confirm_verification_date, "%d-%m-%Y %H:%i:%s") as txt_confirm_verification, verification_code, "" as detail, verification_description, user_created.name as user_input, user_confirm.name as user_verifikasi');

        $crud->set_select_table('idverification, txt_date_input, txt_confirm_verification, verification_code, detail, verification_description, user_input, user_verifikasi');
        $crud->set_sorting('verification_code, detail, verification_description, user_input, user_verifikasi');
        $crud->set_filter('verification_code, detail, verification_description, user_input, user_verifikasi');
		$crud->set_id($this->controller);
    ...
    function search_realisasi_anggaran() {
		...
		$this->db->select('transaction.idtransaction, transaction_detail.idpaket_belanja as id, concat(transaction.transaction_code, " - ", paket_belanja.nama_paket_belanja) as text');
    ...
    function select_paket_belanja() {
		...
		$this->db->join('transaction_detail', 'transaction.idtransaction = transaction_detail.idtransaction');
		$this->db->join('paket_belanja', 'transaction_detail.idpaket_belanja = paket_belanja.idpaket_belanja');
		$this->db->where('transaction_detail.idpaket_belanja', $id);
		$this->db->where('transaction.status', 1);
		$this->db->where('transaction.transaction_status = "INPUT DATA" ');
		$this->db->where('transaction_detail.status', 1);
    ...
    private function generate_transaction_code() {
		...
		if ($data->num_rows() == 0) {
			...
			while($ok == 0) {
				...
				$verification_code = 'ON'.Date('Ymd').$numb;
