...
class Master_urusan_pemerintah extends CI_Controller {
	...
	public function index(){
		...
		$filter = $this->load->view('urusan_pemerintah/vf_urusan_pemerintah', '', true);
		$crud->set_top_filter($filter);

		$v_modal = $this->load->view('urusan_pemerintah/v_urusan_pemerintah', '', true);
		$crud->set_form('form');
	...
	function custom_style($key, $value, $data) {
		...
		if ($key == 'action') {
			...
			$btn .= '<button class="btn btn-info btn-xs btn-copy btn-edit-master_kegiatan" data_id="'.$idurusan_pemerintah.'"><i class="fa fa-file"></i> Copy</button>';
	...
	public function save(){
		...
		$this->form_validation->set_rules('tahun_anggaran_urusan', 'Tahung Anggaran', 'required|trim|max_length[200]');
