...
class Budget_realization extends CI_Controller {
	...
	function add_realization() {
		...
		// validasi apakah uraian belanja dipilih ulang (data diinput lebih dari 1 kali)
		if ($err_code == 0) {
			$this->db->where('budget_realization_detail.idpurchase_plan_detail', $data_idpurchase_plan_detail);
			...
			$this->db->select('budget_realization.idbudget_realization, budget_realization_detail.idbudget_realization_detail');
			$ppd = $this->db->get('budget_realization_detail');
			// echo "<pre>"; print_r($this->db->last_query());die;

			if ($ppd->num_rows() > 0) {
				$err_code++;
				$err_message = "Uraian belanja sudah pernah direalisasikan.";
			}
		}
	...
	function delete_order() {
		...
		if ($is_delete) {
			...
			if ($data_contract->num_rows() > 0) {
				...
				update_status_purchase_plan($the_filter, false);
			}
	...
	function save_realization() {
		...
		if ($err_code == 0) {
			...
			// update status kontrak pengadaan
			update_status_purchase_contract($the_filter, false);
		}
	...
	function excel() {
		...
		if (strlen($iduser_created) > 0) {
			$this->db->where('budget_realization.iduser_created = "' . $iduser_created . '"');
		}

		$this->db->where("budget_realization.status = 1");
		$this->db->where("budget_realization.realization_status != 'DRAFT' ");

		$this->db->join('user', 'budget_realization.iduser_created = user.iduser');
		$this->db->join('budget_realization_detail', 'budget_realization_detail.idbudget_realization = budget_realization.idbudget_realization');
		$this->db->join('contract_detail', 'contract_detail.idcontract_detail = budget_realization_detail.idcontract_detail');
		$this->db->join('contract', 'contract.idcontract = contract_detail.idcontract');
		$this->db->join('purchase_plan', 'purchase_plan.idpurchase_plan = contract_detail.idpurchase_plan');
		...
		$file_excel = APPPATH . "assets/excel/rekap_realisasi_anggaran.xlsx";
		...
		foreach ($data->result() as $key => $value) {
			...
			$sheet->setCellValue("C" . ($start_row + $i), $value->realization_code);
			$sheet->setCellValue("D" . ($start_row + $i), $value->contract_code);
			...
			$sheet->setCellValue("H" . ($start_row + $i), $value->total_realization_detail);
			$sheet->setCellValue("I" . ($start_row + $i), $value->realization_status);
			$sheet->setCellValue("J" . ($start_row + $i), $value->user_name);
			$i++;
		}

		$sheet->getStyle("A" . $start_row . ":J" . ($start_row + $i - 1))->applyFromArray($styleArray11);

		// OUTPUT
		$filename = 'Rekap Realisasi Anggaran ' . date('d-m-Y H-i-s') . '.xlsx';
